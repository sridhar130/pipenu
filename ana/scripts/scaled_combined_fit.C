...
void scaled_combined_fit()
{
 ...
 hist1->Rebin(2);
 double scale_sig = gPipenu->GetChannel("bpip4b0s51r0105")->NormSF();
 double scale_sig_day = scale_sig*1e16;
 // hist1->Scale(scale_sig/hist1->Integral(),"width");
 hist1->Scale(scale_sig);
 hist1->Scale(1e16);
 ...
 hist2->Rebin(2);
 double scale_bg = gPipenu->GetChannel("bmup4b0s66r0100")->NormSF();
 double scale_bg_day = scale_bg*1e16;
 // hist2->Scale(scale_bg/hist2->Integral(),"width");
 hist2->Scale(scale_bg);
 hist2->Scale(1e16);
 hist2->Add(hist1);
...
// hist2->Fit("total","L","",61.0,72.0);
 //hist2->Fit("total","SQ","",60.,72.0);
 // total->Draw("same");

TF1* total1 = new TF1("total1","combo_fit",60.,72.0,8);

  total1->SetParameter(0,1.15);
  total1->SetParameter(1,-0.1817);
  total1->SetParameter(2,16.37);

  total1->SetParameter(7,268.3);
  //total1->SetParLimits(7,267.0,270.0);
  total1->SetParameter(3,68.93);
  total1->SetParLimits(3,67.,69.5);
  total1->SetParameter(4,0.32);
  total1->SetParameter(5,0.46);
  total1->SetParameter(6,51.82);
  
  hist2->Fit("total1","I","",60.,72.0);
  total1->Draw("same");
