...
#------------------------------------------------------------------------------
        self.add_dataset(Dataset('generator'                          ,'bpim1b0s00r0000','local'))
#------------------------------------------------------------------------------
...
# S2: use s1 from the BPIM0B0 
#------------------------------------------------------------------------------
        self.add_dataset(Dataset('generator'                          ,'bpim1b0s00r0000','local'))
        self.add_dataset(Dataset('sim.mu2e.bpim0b0s1xr0000.pipenu.art','bpim1b0s1xr0000','local'))
        self.add_dataset(Dataset('sim.mu2e.bpim0b0s11r0000.pipenu.art','bpim1b0s11r0000','local'))
#------------------------------------------------------------------------------
# 3. input for stage3: stopped pion datasets produced at stage2 : 21:tgt, 22:oot 24:deg
#------------------------------------------------------------------------------
        self.add_dataset(Dataset('sim.mu2e.bpim1b0s21r0000.pipenu.art','bpim1b0s21r0000','local'))
        self.add_dataset(Dataset('sim.mu2e.bpim1b0s24r0000.pipenu.art','bpim1b0s24r0000','local'))
        self.add_dataset(Dataset('sim.mu2e.bpim1b0s25r0000.pipenu.art','bpim1b0s25r0000','local'))
# concatenated versions of s21, s24, s25:
        self.add_dataset(Dataset('sim.mu2e.bpim1b0s2ar0000.pipenu.art','bpim1b0s2ar0000','local'))
        self.add_dataset(Dataset('sim.mu2e.bpim1b0s2dr0000.pipenu.art','bpim1b0s2dr0000','local'))
        self.add_dataset(Dataset('sim.mu2e.bpim1b0s2er0000.pipenu.art','bpim1b0s2er0000','local'))
        return;
...
#------------------------------------------------------------------------------
    def __init__(self,idsid=None):
        ...
        ProjectBase.__init__(self,project='pipenu',family_id='bpim1b0',idsid=idsid);
        self.init_datasets();
#------------------------------------------------------------------------------
# S1 10^8 proton interactions in the PT, half field in the DS
#------------------------------------------------------------------------------        
        s                            = self.new_stage('s1');
        job                          = s.new_job('sim','bpim1b0s00r0000');
...
#------------------------------------------------------------------------------  
        job                          = s.new_job('stn_beam','bpim1b0s11r0000');
...
#------------------------------------------------------------------------------        
        s                            = self.new_stage('s2');
        job                          = s.new_job('sim','bpim1b0s1xr0000');
...
#------------------------------------------------------------------------------        
        ...
        odsid = 'undefined'
        if (os < 'a') : odsid        = self.fFamilyID+s.name()+"%x"%int(os)+'r0000'; # don't change
...
#------------------------------------------------------------------------------  
        job                          = s.new_job('gen_sim_deg','bpim1b0s24r0000');
...
#------------------------------------------------------------------------------
# stage 4
# s4:digi_trig : InputDsID is 'bpim1bos31r0000' or 'bpim1b0s34r0000'
