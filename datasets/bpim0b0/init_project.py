...
#------------------------------------------------------------------------------
        self.add_dataset(Dataset('generator'                          ,'bpim0b0s00r0000','local'))
#------------------------------------------------------------------------------
# 2. input for stage2 : datasets produced by stage1
#    's1x' - concatenated output of the BeamFilter, 's11'
#    for 2.5e8 generated events concatenation should yield < 2GBytes, of just one file
#------------------------------------------------------------------------------
        self.add_dataset(Dataset('sim.mu2e.bpim0b0s11r0000.pipenu.art','bpim0b0s11r0000','local'))
        self.add_dataset(Dataset('sim.mu2e.bpim0b0s12r0000.pipenu.art','bpim0b0s12r0000','local'))
        self.add_dataset(Dataset('sim.mu2e.bpim0b0s1xr0000.pipenu.art','bpim0b0s1xr0000','local'))
#------------------------------------------------------------------------------
# 3. input for stage3: stopped pion datasets produced at stage2 : 21:tgt, 22:oot 24:deg
#------------------------------------------------------------------------------
        self.add_dataset(Dataset('sim.mu2e.bpim0b0s21r0000.pipenu.art','bpim0b0s21r0000','local'))
        self.add_dataset(Dataset('sim.mu2e.bpim0b0s24r0000.pipenu.art','bpim0b0s24r0000','local'))
        self.add_dataset(Dataset('sim.mu2e.bpim0b0s25r0000.pipenu.art','bpim0b0s25r0000','local'))
# concatenated versions of s21, s24, s25:
        self.add_dataset(Dataset('sim.mu2e.bpim0b0s2ar0000.pipenu.art','bpim0b0s2ar0000','local'))
        self.add_dataset(Dataset('sim.mu2e.bpim0b0s2dr0000.pipenu.art','bpim0b0s2dr0000','local'))
        self.add_dataset(Dataset('sim.mu2e.bpim0b0s2er0000.pipenu.art','bpim0b0s2er0000','local'))
#------------------------------------------------------------------------------
# 4. Input for s4
#------------------------------------------------------------------------------
        self.add_dataset(Dataset('dts.mu2e.bpim0b0s31r0000.pipenu.art','bpim0b0s31r0000','local'))
        self.add_dataset(Dataset('dts.mu2e.bpim0b0s34r0000.pipenu.art','bpim0b0s34r0000','local'))
#------------------------------------------------------------------------------
# 5. Input for s5 reco
#------------------------------------------------------------------------------
        self.add_dataset(Dataset('dig.mu2e.bpim0b0s41r0000.pipenu.art','bpim0b0s41r0000','local'))
        self.add_dataset(Dataset('dig.mu2e.bpim0b0s44r0000.pipenu.art','bpim0b0s44r0000','local'))
#------------------------------------------------------------------------------
# 5. Input for s5 stntupling
#------------------------------------------------------------------------------
        self.add_dataset(Dataset('mcs.mu2e.bpim0b0s51r0000.pipenu.art','bpim0b0s51r0000','local'))
        self.add_dataset(Dataset('mcs.mu2e.bpim0b0s51r0100.pipenu.art','bpim0b0s51r0100','local'))
        self.add_dataset(Dataset('mcs.mu2e.bpim0b0s54r0100.pipenu.art','bpim0b0s54r0100','local'))
        return;
...
#------------------------------------------------------------------------------
    def __init__(self,idsid=None):
        ...
        ProjectBase.__init__(self,project='pipenu',family_id='bpim0b0',idsid=idsid);
        self.init_datasets();
#------------------------------------------------------------------------------
# S1 10^8 proton interactions in the PT, half field in the DS
#------------------------------------------------------------------------------        
        s                            = self.new_stage('s1');
        job                          = s.new_job('sim','bpim0b0s00r0000');
...
#------------------------------------------------------------------------------
# s1:concat: concatenation of the input dataset, has to be defined ! 
#------------------------------------------------------------------------------        
        ...
        odsid                        = self.fFamilyID+s.name()+'x'+'r0000'; # this is concatenation...
...
#------------------------------------------------------------------------------  
        job                          = s.new_job('stn_beam','bpim0b0s11r0000');
...
#------------------------------------------------------------------------------        
        ...
        odsid25                      = self.fFamilyID+s.name()+'5'+'r0000';

        job.fOutputStream            = ['TargetStopOutput' , 'DegraderStopOutputCH2', 'DegraderStopOutputPb' ]
        job.fOutputDsID              = [odsid21            ,  odsid24               ,  odsid25               ]
        job.fOutputFnPattern         = ['sim.mu2e.'+odsid21,  'sim.mu2e.'+odsid24   ,  'sim.mu2e.'+odsid25   ]
        job.fOutputFormat            = ['art'              ,  'art'                 ,  'art'                 ]
#------------------------------------------------------------------------------
# s2:stn_tgt
#------------------------------------------------------------------------------  
        job                          = s.new_job('stn_tgt','bpim0b0s21r0000');
...
# s2:stn_deg
#------------------------------------------------------------------------------  
        job                          = s.new_job('stn_deg','bpim0b0s24r0000');
...
#------------------------------------------------------------------------------  
        job                          = s.new_job('gen_sim_deg','bpim0b0s24r0000');
...
#------------------------------------------------------------------------------
# stage 4
# s4:digi_trig : InputDsID is 'bpim0b0s31r0000' or 'bpim0b0s34r0000'
